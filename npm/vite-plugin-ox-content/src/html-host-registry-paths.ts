...
export function toHtmlHostClientModuleId(moduleId: string, root = process.cwd()): string {
  const [pathname, suffix = ""] = splitModuleSuffix(moduleId);
  if (isBareHtmlHostSpecifier(pathname) || pathname.startsWith("/@fs/")) {
...
export function resolveHtmlHostDocumentPath(documentPath: string, root: string): string {
  return path.isAbsolute(documentPath)
    ? stripViteQuery(documentPath)
    : path.resolve(root, stripViteQuery(documentPath));
}

export function resolveHtmlHostWatchFile(file: string, root: string): string {
  return path.isAbsolute(file) ? stripViteQuery(file) : path.resolve(root, stripViteQuery(file));
}

export function shouldInvalidateHtmlHostRegistry(
  file: string,
  root: string,
  ...
  if (extraWatchFiles?.some((watchFile) => changed === resolveHtmlHostWatchFile(watchFile, root))) {
    return true;
  }
...
export function isBareHtmlHostSpecifier(moduleId: string): boolean {
